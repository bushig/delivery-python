...
def test_volume_create_success():
    ...
    assert result.get_value() == Volume(_value=Decimal(10))


def test_same_volumes_equal():
    assert Volume(_value=Decimal(1)) == Volume(_value=Decimal(1))
    assert Volume(_value=Decimal("2.5")) == Volume(_value=Decimal("2.5"))


def test_volume_mutation_forbidden():
    volume = Volume(_value=Decimal(1))
    with pytest.raises(FrozenInstanceError):
        volume._value = Decimal(2)


@pytest.mark.parametrize(
    "volume_1, volume_2",
    [
        pytest.param(Volume(_value=Decimal(1)), Volume(_value=Decimal(2)), id='different_values'),
        pytest.param(Volume(_value=Decimal("2.5")), Volume(_value=Decimal("3.5")), id='different_decimal_values'),
        pytest.param(Volume(_value=Decimal(1)), Volume(_value=Decimal("1.5")), id='integer_vs_decimal'),
    ]
)
...
def test_volume_add():
    v1 = Volume(_value=Decimal(10))
    v2 = Volume(_value=Decimal(5))
    result = v1 + v2
    assert result == Volume(_value=Decimal(15))
...
def test_volume_sum_volumes():
    volumes = [
        Volume(_value=Decimal(10)),
        Volume(_value=Decimal(5)),
        Volume(_value=Decimal(3)),
    ]
    result = Volume.sum_volumes(volumes)
    assert result == Volume(_value=Decimal(18))
